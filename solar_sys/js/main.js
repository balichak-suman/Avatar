...
function initGui() {
    ...
    window.changeCamera = function (val) {
        // Highlight button
        $('.planet-btn').removeClass('active');
        // Find button with text val and add active? simple check
        $('.planet-btn').filter(function () { return $(this).text() === val; }).addClass('active');
...
function init() {
    ...
    stats = new Stats();
    gui = new dat.GUI();
    gui.close(); // Keep it closed or hide it?
    gui.domElement.style.display = 'none'; // Hide completely by default
