CelestialBody.prototype.updateOrbitAndRotation = function (time) {
    ...
    if (this.parent != null) {
        ...
        if (this.isComet) {
            ...
            // Real-time rotation for Earth
            if (this.name === "Earth" && typeof useRealTime !== 'undefined' && useRealTime) {
                // Calculate Greenwich Mean Sidereal Time (GMST)
                // GMST approximation: 18.697374558 + 24.06570982441908 * D
                // Where D is days since J2000.0
                var JD = globalTime.getJulianDate();
                var D = JD - 2451545.0;
                var GMST = 18.697374558 + 24.06570982441908 * D;
                GMST = GMST % 24;

                // Convert to radians (1 hour = 15 degrees)
                var angle = (GMST * 15) * (Math.PI / 180);

                // Apply rotation (adjusting for texture offset if needed)
                // We set rotation.y directly instead of rotating incrementally
                this.objectGroup.rotation.y = angle + (this.rotation.meridianAngle * Math.PI / 180);
            } else {
                // Standard incremental rotation
